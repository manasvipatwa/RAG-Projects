...
from guardrails.hub import LlmRagEvaluator, HallucinationPrompt, QACorrectnessPrompt
...
def prepare_dataframe(predefined_question, relevant_doc, generated_answer):
    dataframe = pd.DataFrame(
        [
            {
                "input": predefined_question,
                "reference": relevant_doc,  # The context document used to generate the answer
                "output": generated_answer,  # The answer generated by the LLM
            }
        ]
    )
    return dataframe
...
def validate_answer(predefined_question, relevant_doc, generated_answer):
    # Prepare the dataframe with the question, reference (document), and generated answer
    dataframe = prepare_dataframe(predefined_question, relevant_doc, generated_answer)

    # Use the QA correctness evaluator
    ...
    # st.write(Q_and_A_classifications)
    # Extract the result of the evaluation
    result = Q_and_A_classifications['label'][0] 
    
    # Return the validated answer or raise an exception if incorrect
    if result == "correct":
        return generated_answer  
    else:
        raise ValueError("Generated answer is incorrect.")
...
if st.session_state["section"] == "document_upload":
    ...
    if uploaded_files:
            ...
            for j, predefined_question in enumerate(example_questions[start_idx:end_idx]):
                with cols[j]:
                    if st.button(predefined_question):
                        ...
                        validated_answer = validate_answer(predefined_question, relevant_doc, answer)
                        answers.append({
                            'question': predefined_question,
                            'file': file_name,
                            'distance': distance,
        ...
        if user_question:
            ...
            validated_answer = validate_answer(user_question, relevant_doc, answer)
            answers.append({
        ...
        if answers:
            ...
            for ans in answers:
                ...
                # st.write(f"**Validated Answer:** {ans['validated_answer']}")
