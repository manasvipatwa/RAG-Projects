...
if st.session_state["section"] == "document_upload":
    ...
    if uploaded_files:
        ...
        agent = Agent(
            ...
            markdown=True,
            description="You are a QA agent that answers questions based on uploaded documents.",
            instructions=["For each question, retrieve the most relevant document, extract the answer, and provide it."],
        )
            ...
            for j, predefined_question in enumerate(example_questions[start_idx:end_idx]):
                with cols[j]:
                    if st.button(predefined_question):
                        ...
                        # Print the answer in markdown format
                        # agent.print_response(predefined_question)
                        answers.append({
                            'question': predefined_question,
                            'file': file_name,
                            'distance': distance,
    ...
    if url_input:
        ...
        if documents:
            ...
            agent = Agent(
                model=OpenAIChat(id="gpt-4o"),
                markdown=True,
                description="You are a QA agent that answers questions based on website content.",
                instructions=["For each question, retrieve the most relevant section of the website, extract the answer, and provide it."],
            )
